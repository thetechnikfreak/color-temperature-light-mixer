...
class TemperatureMixerLight(LightGroup, RestoreSensor):
    ...
    def async_update_group_state(self) -> None:
        ...
        self._attr_brightness = (
            int(min(sum(brightnesses), 255)) if brightnesses else None
        )
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        _LOGGER.debug("%s: turn on with params: %s", self._cached_friendly_name, kwargs)
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        _LOGGER.debug(
            "%s: invoking turn_off for the light group", self._cached_friendly_name
        )
